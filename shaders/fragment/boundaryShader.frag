...
void main()
{
  ...
  float totalWaterIn = water[TOTAL]; // water[TOTAL] before precipitation feedback modifies it below
    ...
    if (wall[VERT_DISTANCE] <= 8) { // within height of buildings
      ...
      case WALLTYPE_LAND:
        if (wall[VERT_DISTANCE] <= wallVerticalInfluence) {
          ...
          // Use totalWaterIn (the value this iteration started with), so that this
          // evaporation is exactly the amount the surface wall cell below removes
          // from its soil moisture. Using the already precipitation-adjusted
          // water[TOTAL] made the two sides diverge whenever rain formed or
          // evaporated in this cell, manufacturing or destroying water.
          float evaporation = calcEvaporation(realTemp, totalWaterIn, float(wall[VEGETATION]), waterInSurface[SOIL_MOISTURE]) / influenceDevider;
